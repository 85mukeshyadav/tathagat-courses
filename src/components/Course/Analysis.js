...
const Analysis = () => {
    ...
    const updateSeries = () => {
      ...
      useEffect( async()=>{
        ...
        const resp = await axios.post(process.env.REACT_APP_API + '/test-analysis', param, options);
        console.log("11111",resp.data)
        setAnalysisData(resp.data);
        
        const respRep = await axios.post(process.env.REACT_APP_API + '/report-user-stand', param, options);
        ...
        <div className="relative px-4 anal-tab">
                <div className="grp-sec"> 
                    ...
                    { getAnalysisData?.section?.map((res, i) => (
                        <>
                    <div key={i}>
                    <div className='grp-sec-name' style={{marginTop: '50px'}}>Section Name - {res.sectionName} </div>
                    ...
                    <table className='' style={{width:'100%'}}>
                        ...
                        <tr key={`${i}-${ind}`} className='bg-gray-200'>
                            ...
                            <td className='text-left p-2'>{ques.topicName ? ques.topicName : '--' }</td>
                            <td className='text-left p-2'>{ques.answer_status ? ques.answer_status : '--'}</td>
                            <td className='text-left p-2'>{ques.optionType == 'input' ?  (ques.usersAnswer ? ques.usersAnswer : '--' ) : (ques.user_answer != -1 ? ques.user_answer : '--')}</td>
                            <td className='text-left p-2'>{ques.correctoption ? ques.correctoption : '--'}</td>
                        </tr>
                        ))}
                    ...
                    <table className='' style={{width:'100%', marginTop: '15px', marginBottom: '100px', borderBottom: '1px solid red'}}>
                        <tbody>
                        <tr className='bg-blue-300'>
                        ...
                        <tr className='bg-gray-200'>
                            <td className='text-left p-2'>{chap.chapterName}</td>
