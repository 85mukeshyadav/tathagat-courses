...
import { FiBook, FiChevronDown, FiFile, FiMinusCircle, FiPlay, FiVideo } from 'react-icons/fi';
...
const CourseDetails = (props) => {
    ...
    useEffect(async () => {
        ...
        const packRes = await axios.get(process.env.REACT_APP_API + '/getPackageDetails/' + localStorage.getItem('pkgid'), {params:{userId: localStorage.getItem('user')}})
        ...
        let finalTestResult = [];
            ...
            finalTestResult = finalTestResult.map((d,i)=>{
                let calPer = parseInt(((d.testAttempted / d.testDataCount) * 100).toFixed());
                d.chartOptions['series'][0]['dataLabels']['format'] = d.testAttempted +' / ' + d.testDataCount;
    ...
    const getTestOfRelatedSub = async (testData, index) =>{
        ...
        if(finaTes[index]['chapterData'] && finaTes[index]['chapterData'].length > 0) {
            return;
        }
        ...
        const getTest = await axios.post(process.env.REACT_APP_API + '/getTestForTree', param);
    ...
    },[getCourseList,getCourseTest, getSubjectList, getSubjectTest, getIndex, getChapterList, getChapterTest, getTopicList, getTopicTest])
    ...
    const _getSubjectList = async (id, index) => {
        const res = await axios.get(process.env.REACT_APP_API + '/getSubjectList/' + localStorage.getItem('pkgid')+'/'+ id, {params:{userId: localStorage.getItem('user')}})
    ...
    const _getChapterList = async (id, index) => {
        const res = await axios.get(process.env.REACT_APP_API + '/getChaperList/' + localStorage.getItem('pkgid')+'/'+ id, {params:{userId: localStorage.getItem('user')}})
    ...
    const _getTopicList = async (id, index) => {
        const res = await axios.get(process.env.REACT_APP_API + '/getTopicList/' + localStorage.getItem('pkgid')+'/'+ id, {params:{userId: localStorage.getItem('user')}})
    ...
    const _getTopicTest = async (id, index) => {
        const res = await axios.get(process.env.REACT_APP_API + '/getTopicTestList/' + localStorage.getItem('pkgid')+'/'+ id, {params:{userId: localStorage.getItem('user')}})
        setTopicTest(res.data.testList);
        let ind = {...getIndex};
        // console.log("c", ind)
    ...
    return (
        ...
        <div className="bg-white">
            <div className="bg-cover bg-center h-[400px] before:bg-[#000000f1] before:opacity-80 before:absolute before:left-0 before:w-screen
             before:h-[400px] before:z-0 opacity-90 "
                style={{ backgroundImage: data && data?.thumbnail != undefined ? `url(${data.thumbnail})` : 'url(https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=870&q=80)' }}>
                <div className='z-[1] relative pt-20 items-center flex justify-center flex-col'>
                    <h1 className='text-white font-bold text-4xl uppercase'>{data && data.PackageName}</h1>
                    <p className='text-white pt-8 w-1/2'>{data && data.PackageDesc}</p>
                    {!isPurchased && <div className={courseType == 'allCourse' ? '' : 'displayProp'}><button onClick={() => {
                        if (!isAuth) {
                            navigate('/signin')
                        } else if (isAuth) {
                            setPurchased(false);
                        ...
                        ₹{data && data.price} Buy now </button></div>}
                </div>
            </div>

            {((url !== '' && courseType == 'myCourse')) ? <VideoPlayer url={url} title={title} desc={desc} /> : null}
            ...
            <div className={courseType == 'myCourse' ? '' : 'displayProp'}>
                ...
                <div className='flex justify-between mb-4' style={{marginLeft: '8%'}}>
                    <div className='w-10/12 mr-4 m-8 mt-20 '>
                        <div className="cource-details-section">
                            <ul className='flex'>
                                <button onClick={() => { setCurrentActiveTab('overview') }}
                                    className={`flex items-center border-2  p-1 pl-4 pr-4 disabled:cursor-not-allowed rounded-sm font-semibold ${currentActiveTab == 'overview' ? 'bg-blue-400 text-gray-50' : 'bg-gray-100 text-gray-500'} `}>
                                    Overview
                                </button>
                                ...
                                </button> */}
                                {
                                    (
                                        getPckRelatedTest.map((sub, i)=>(
                                            <button key={i} onClick={() => {getTestOfRelatedSub(sub.testData, i); setCurrentActiveTab(sub.subjectName) }}
                                            className={`flex items-center border-2 ml-2 p-1 pl-4 pr-4 disabled:cursor-not-allowed rounded-sm font-semibold ${currentActiveTab == sub.subjectName ? 'bg-blue-400 text-gray-50' : 'bg-gray-100 text-gray-500'}`}>
                                            {sub.subjectName}
                                            </button>
                                        ))
                                    )
                        ...
                        <div className='overViewSection'>
                            <div className='p-4' style={{ display: currentActiveTab == 'overview' ? 'block' : 'none' }}>
                                <div className={`line`}>
                                    <div className={`h-1`}>About this package 
                                   </div>
                                ...
                                <div style={{marginTop: "10px"}}>
                                    ...
                                    <div style={{textAlign:'center'}}>  { getPckRelatedTest.length == 0 ? <span className="bt-spinner"></span> : 
                                    ''}
                                     ...
                                     <ul>
                                        {(
                                            getPckRelatedTest.map((sub, i) => (
                                                <li key={i} style={{fontSize: '25px',
                                                    marginTop: '20px',
                                                    padding: '10px'
                                                    }}> 
                                                    <span style={{marginRight:'40px'}}>{sub.subjectName} </span>
                                                    <span style={{marginRight:'20px'}}>|</span> 
                                                    <span style={{marginLeft:'20px', fontSize: '20px'}}>No. of test Assign:- {sub?.testData.length}</span>
                            ...
                            
                            {
                                (
                                    getPckRelatedTest.map((sub, i) => (
                                        <div key={i} className='p-4' style={{ display: currentActiveTab == sub.subjectName ? 'block' : 'none' }}>
                                            <div>
                                            {/* <div className={`h-3 mb-4`}>Tests<span className="bt-spinner"></span></div> */}
                                            <TreeList subData={sub.testData} chapData={sub.chapterData} setNodeClick={setNodeClick}/>
                                            </div>

                                            <div style={{textAlign:'center'}}>  { sub.chapterData.length == 0 ? <span className="bt-spinner"></span> : 
                                                ''}
                        ...
                        {Similardata.map((res, i) => (
                            <div className="max-w-6xl mx-auto cursor-pointer" >
                                <div className="flex items-center justify-center">
                                    <div onClick={() => { setData([res]); window.scrollTo(0, 0); }} className="max-w-sm xl:w-full xl:py-6 px-3">
                                        <div className="bg-white hover:shadow-xl border-gray-50 border-4 rounded-lg overflow-hidden">
                                            ...
                                            <div className="p-4">
                                                ...
                                                <p className="text-gray-700">8.5k+ Students Enrolled </p>
                                            </div>
                                            ...
                                            <button className='px-4 pt-3 w-full shadow-inner pb-4 border-t border-gray-300 items-center bg-indigo-500 
                                                text-white text-xl font-bold'>Buy now</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
