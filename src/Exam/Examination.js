...
import { FaCalculator, FaEquals, FaFile, FaInfo, FaPage4 } from 'react-icons/fa';
...
const Examination = React.memo(() => {
    ...
    const [currentQuesStatus, setCurrentQuesStatus] = useState({
        answered: [], notAnswered: [], notVisited: [], markForReview: [], bothAnsReview: []
    });
    ...
    useEffect(async () => {
        ...
        if (selectedSectionnumber > 0) {
            ...
            let newJson = {
                totalMarks: questionAttempt ? (questionAttempt * positiveMarks) : (Question.length * positiveMarks) || 0,
                correctAnswers,
                wrongAnswers,
                ...
                answered, notAnswered, notVisited, markedReview, bothAnsReview,
                question
            }
        ...
        const res = await axios.get(process.env.REACT_APP_API + '/gettest/' + localStorage.getItem('testid'), options)
        ...
        setQuestionAttempt(res.data[0].Section[selectedSectionnumber].questionAttempt);
        setdata(res.data[0].Section)
        setcurrentIndex(0);
        setAns('');
        setExamLevel(res.data[0].examLevel)
        setTestName(res.data[0].TestTitle)
        let objArray = { notAnswered: [], notVisited: [], answered: [], markForReview: [], bothAnsReview: [] };

        let quesAnsArray = [];
        setTotalSection(res.data[0].Section.length);
        res.data[0].Section[0] && res.data[0].Section.map((res, index) => {
    ...
    function notViewSection(data, i) {
        ...
        let newJson = {
            totalMarks: data.Section[i].questionAttempt ? (data.Section[i].questionAttempt * positiveMarks) : (data.Section[i].QuestionList.length * data.Section[i].positiveMarks) || 0,
            correctAnswers,
            wrongAnswers,
    ...
    const submitTest = async (full_attempt, sectionData=[]) => {
        ...
        let sec = sectData.map((d,i)=>{
            ...
            d['score'] = (d.correctAnswers * d.positiveMarks) - (calWrong * Math.abs(d.negativeMarks))
            netScore = netScore + (d.correctAnswers * d.positiveMarks) - (calWrong * Math.abs(d.negativeMarks));
            return d;
        })
        ...
        const res = await axios.post(process.env.REACT_APP_API + '/submitTest', params, options);
            ...
            <main className="absolute w-full min-h-screen z-10 bg-gray-50 text-gray-900 font-sans overflow-x-hidden">
                    ...
                    <div>
                        <table className='min-w-[98vw]'>
                            ...
                            <tr className='bg-blue-300'>
                                ...
                                <th className='text-left p-2'>No of questions to answer</th>
                                ...
                                <th className='text-left p-2'>Answered & Marked for Review (will be considered for evaluation)</th>
                ...
                <footer className='absolute border-t-2 border-gray-400 flex w-full justify-center self-center bottom-[2%]'>
                    <div className=''>
                        ...
                        <button onClick={() => redirectTo()} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-6 py-2 mr-10'>
                            Ok
                        </button>
                        </>
                        : 
                        <>
                        <p>Are you sure to submit this Group? Click 'Yes' to proceed; Click 'No' to go back.
                            <br></br>
                            Dear Candidate, Once the Group is submitted, you cannot edit your responses.
                        </p>
                        <button onClick={() => {submitTest(1); redirectTo();}} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-6 py-2 mr-10'>
                            Yes
                        </button>
                        
                        <button onClick={() => {setLeft(0); setFinishExam(false)}} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-6 py-2 mr-10'>
                            No
                        </button>
                        </>
                        }
    ...
    return (
        ...
        <div className="bg-gray-50 min-h-screen h-full">
            ...
            {Alert ?
                        ...
                        <div className="bg-white rounded-lg md:max-w-md md:mx-auto p-4 fixed inset-x-0 bottom-0 z-50 mb-4 mx-4 md:relative">
                            <div className="md:flex items-center">
                                ...
                                <div className="mt-4 md:mt-0 md:ml-6 text-center md:text-left">
                                    ...
                                    <p className="text-sm text-gray-700 mt-1">once You leave the section, This action cannot be undone.
                                    </p>
                                </div>
                            </div>
                ...
                : null}
                {
                (questionAttempt && Question.length > questionAttempt) ? <div className="importantNote">
                        <b>You cannot attempt more than {questionAttempt} questions in this section.</b>
                    </div> : ""
                }

            <nav className='flex justify-between bg-slate-600 p-2'>
                <div className='text-yellow-400 font-semibold'>{SectionName}</div>
                <div className='text-yellow-400 font-semibold'>Test Name:- {getTestName}</div>
                <div className='flex items-center'>

                    <button onClick={togglePopup} className='flex items-center text-gray-50 mr-6'>
                        Calculator</button>
                    ...
                    <button onClick={setModalIsOpenToTrue} className='flex items-center text-gray-50 mr-6'>
                        <div className='bg-green-400 p-2 rounded-full mr-2'>
                            <FaFile className=' text-white' />
                        </div>
                        Question paper</button>
                    <button onClick={setModalInsOpenToTrue} className='flex items-center text-gray-50'>
                    ...
                    <div class="quesHeight">
                        {
                            ...
                            Question.map((allQues, i) => (
                                <div key={i}>
                                    <span>Q{i + 1}</span><p key={i} className='w-10/12 p-4' dangerouslySetInnerHTML={{
                                        __html: (allQues.questionType == "paragraph" ? allQues.paragraph : allQues.question)
                                    }}></p>
                                </div>
                            ))
                    ...
                    <button onClick={setModalInsOpenToFalse}>x</button>
                        {
                            ...
                            <embed src={pms} type="application/pdf"   height="700px" width="600px"></embed> :
                            (getExamLevel == 1) ? 
                            <embed src={fullLength} type="application/pdf"   height="700px" width="600px"></embed> :
                            <embed src={copyCat} type="application/pdf"   height="700px" width="600px"></embed>
                        }
                </Modal>
            </nav>
            ...
            <div id='quesAns' className='flex justify-between'>
                <Modal isOpen={modalQuesAlertOpen} style={customStyles} ariaHideApp={false}>
                    <div style={{display: 'flex', flexDirection: 'row-reverse'}}><button onClick={setModalQuestAttemptAlert}>x</button></div>
                    <p style={{textAlign: 'center'}}><b>You cannot attempt more than {questionAttempt} questions in this section.</b></p>
                    <p style={{textAlign: 'center'}}>Please clear the response for some other question of this section to attempt this question</p>
                    <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'center', marginTop: '20px'}}>
                    <button style={{marginRight: '20px'}} onClick={() => {setModalQuestAttemptAlert() }} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-4 py-2'>
                            Resume
                        </button>
                        <button style={{marginLeft: '20px'}} onClick={() => { setLeft(1); _submitPreTest(); setFinishExam(true);}} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-4 py-2'>
                            Submit
                        </button>
                    </div>
                </Modal>
                <div className='w-10/12 mr-4'>
                        ...
                        <ul className='flex'>
                            {data.map((res, i) => (
                                <button key={i} onClick={() => {
                                    if (selectedSectionnumber < i)
                                        setAlertbox(false)
                                }} className={`flex items-center border-2 ml-2 p-1 pl-4 pr-4 disabled:cursor-not-allowed rounded-sm ${selectedSectionnumber == i ? 'bg-blue-400' : 'bg-gray-100'} ${selectedSectionnumber == i ? 'text-gray-50' : 'text-gray-500'} font-semibold`}
                    ...
                    <div className='flex justify-between border-y-2 border-gray-300 pl-2'>
                        <p>section</p>
                        <p className='flex font-bold'>Time left:<Counter getLeft={getLeft} setCount={setCount} key={count + 'time'} time={count} changeFinish={changeFinish} /></p>
                    </div>
                    <div className='flex justify-between border-b-2 pl-2'>
                        <p className='font-bold'>Question no {currentQuesIndex + 1}</p>
                        <p>Marks for correct answer {data[selectedSectionnumber]?.positiveMarks} | negative marks <span className='text-red-500'>{data[selectedSectionnumber]?.negativeMarks}</span></p>
                    </div>

                    {Question.map((res, i) => {
                        return currentQuesIndex == i ?
                            <div key={i} className='text-left justify-between overflow-y-scroll max-h-[500px] pl-2' style={{ height: '100%',marginBottom: '63px' }}>
                               
                                {(res.questionType == "paragraph" ?
                                    <>
                                        <div className="prow" style={{ height: '100%' }}>
                                            <div className="pcolumn" style={{ borderRight: '1px solid grey' }}>
                                            ...
                                            <div className="pcolumn">
                                                <>
                                                    ...
                                                    {res.optionType == 'input' ? <div>
                                                        <QuestionInput getMode={getMode} getAns={getAns} setAns={setAns} getQuesAns={getQuesAns} setQuesAns={setQuesAns} index={i} />
                                                    </div>

                                                        : <>

                                                            {res.questionoption[0] && res?.questionoption.map((ans, iAns) => {
                                                                let newObj = { ...currentQuesStatus };
                                                                return <div key={iAns} className="flex items-center space-x-3 cursor-pointer px-4 py-2 rounded-sm">
                                                                    <>
                                                                        <input checked={(newObj['answered'][i] == 1 && iAns == getRadio) ? 'checked' : null} onChange={() => {
                                                                            SelectAnswer(i);
                                                                            setselectedAns(iAns);
                                        ...
                                        <>
                                            ...
                                            {res.questionoption[0] && res?.questionoption.map((ans, iAns) => {
                                                ...
                                                return <div key={iAns} className="flex items-center space-x-3 cursor-pointer px-4 py-2 rounded-sm">
                                                    ...
                                                    <h3 dangerouslySetInnerHTML={{
                                                        __html: ans?.option
                                                    }} className="text-gray-900 group-hover:text-white text-sm font-semibold text-left"  key={ i + iAns + 'p'} >
                ...
                <div className='border-gray-800 border-l-2 h-full'>
                    <div className='flex flex-col justify-center w-80 items-center'>
                        ...
                        <div className='flex flex-wrap gap-4 p-2 mt-2'>
                            ...
                            <div className='flex font-medium items-center self-center'>
                                ...
                                <p className='text-sm font-normal'>Answered & Marked for Review (will be considered for evaluation) </p>
                            </div>
                        </div>
                        <div className='flex flex-col w-full'>
                            <div className='bg-[#4e85c5]'>
                                <p className='text-left p-2 text-white font-semibold w-full'>{SectionName}</p>
                            </div>
                            <p className='text-left pl-2 text-gray-700 font-semibold w-full'>Choose a Question</p>
                        </div>
                ...
                <footer className='fixed w-full border-t-2 border-gray-400 p-2 bg-gray-50 flex justify-between bottom-0'>
                    <div>
                        <button onClick={() => {
                            ...
                            if(questionAttempt <= countQues) {
                                const chbx = document.getElementsByName("ans" + currentQuesIndex);
                            ...
                            //     }
                            // } 
                        }} className='border-2 text-gray-600 hover:bg-[#0c7cd5] hover:text-white rounded-sm border-gray-300 px-4 py-2 mr-2'>
                    ...
                    <div className='flex justify-around items-center w-[37%]'>
                        <button onClick={() => {
                            ...
                            if(newArry[currentQuesIndex]['quesAns'] == -1 && newArry[currentQuesIndex]['optionType'] == 'input') {
                                console.log("1")
                                newArry[currentQuesIndex]['isClicked'] = true;
                                newArry[currentQuesIndex]['state'] = 2;
                                if(getAns){
                                ...
                                if(newArry[currentQuesIndex + 1]['quesAns'] == -1 && newArry[currentQuesIndex]['optionType'] == 'input') {
                                    if(qu.length - 1  > currentQuesIndex) setAns('');
                                }
                                newArry[currentQuesIndex]['quesAns'] = newArry[currentQuesIndex]['quesAns'];
                            }

                            console.log("bbbbbbbb", newArry, newObj, qu.length, currentQuesIndex);
                            
                            if( currentQuesIndex ==  qu.length - 1){
                                if(newArry[currentQuesIndex]['optionType'] != 'input' && newArry[currentQuesIndex]['quesAns'] != -1){
                                ...
                                if(newArry[currentQuesIndex + 1]['optionType'] != 'input' && newArry[currentQuesIndex + 1]['quesAns'] != -1){
                        ...
                        <button onClick={() => { setLeft(1); _submitPreTest(); setFinishExam(true);}} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-4 py-2 mr-10'>
                            Submit
                        </button>
                    </div>
                </footer>
