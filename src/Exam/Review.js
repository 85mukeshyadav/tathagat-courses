            ...
            if (index == selectedSectionnumber) {
                ...
                let newQues = res.QuestionList.map((ques, indexs) => {
                    ...
                    let everyQues = {
                        ...
                        quesAns: respReview.data.section[index].question[indexs].usersAnswer,
                        state: 3,
                        ansStatus: "",
                    ...
                    if(indexs == 0) {
                        if(ques.optionType == 'input') {
                            ...
                            setRadio(respReview.data.section[index].question[0].usersAnswer)
                        }
                    }
                    return ques;
                });
            ...
            <main className="absolute w-full min-h-screen z-10 bg-gray-50 text-gray-900 font-sans overflow-x-hidden">
                ...
                <footer className='absolute border-t-2 border-gray-400 flex w-full justify-center self-center bottom-[2%]'>
                    ...
                    {count <= 0 ? ''
                        ...
                        <button onClick={() => submitTest()} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-6 py-2 mr-10'>
                            Yes
                        </button>
                        <button onClick={() => setFinishExam(false)} className='bg-[#38aae9] text-white hover:border-gray-700 hover:border-2 border-2 rounded-sm px-6 py-2 mr-10'>
                            No
                        </button>
                        </>
                        }
    ...
    const getSolution = async (res) => {
        ...
        if(res.explantation){
           setViewSolution(res.explantation)
            ...
            const resp = await axios.get(process.env.REACT_APP_API + '/question-details/' + res.questionId, options);
            ...
            <div id='quesAns' className='flex justify-between'>
                <div className='w-10/12 mr-4'>
                    ...
                    <div className='flex justify-between border-y-2 border-gray-300 pl-2'>
                        <p>section</p>
                        <p className='flex font-bold'>Time left:<Counter setCount={setCount}  key={count + 'time'} time={count} changeFinish={changeFinish} /></p>
                    </div>
                    ...
                    {Question.map((res, i) => {
                        return currentQuesIndex == i ?
                            <div key={i} className='text-left justify-between overflow-y-scroll max-h-[500px] pl-2' style={{ height: '100%' }}>
                                                        ...
                                                        : <>
                                                            ...
                                                            {res.questionoption[0] && res?.questionoption.map((ans, iAns) => {
                                                                ...
                                                                return <div key={iAns} className="flex items-center space-x-3 cursor-pointer px-4 py-2 rounded-sm">
                                                                    <input checked={(iAns == getRadio) ? 'checked' : null}
                                                                        type="radio" name={`ans` + i} disabled={true}
                                                                    ...
                                                                    {(reviewQues[i].answerStatus == "C" && iAns == getRadio) ?
                                                                        <i className={`mr-1 fa fa-check`}> </i> :
                                                                        (reviewQues[i].answerStatus == "W" && iAns == getRadio) ?
