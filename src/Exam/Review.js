//import liraries
import { Divider, Modal, ScrollArea, Switch, Textarea, Tooltip } from "@mantine/core";
import { IconAlarm, IconClock, IconHelpCircle, IconHourglass } from "@tabler/icons-react";
  ...
  return (
    <div className="bg-gray-50 h-full overflow-hidden">
      <nav className="sm:flex block justify-between bg-slate-600 p-2 py-6">
        ...
        <div className="text-yellow-400 sm:text-lg font-semibold">{getTestName}</div>
        <div></div>
      </nav>
      {/* report modal */}
      <Modal opened={isReportVisible} onClose={() => setIsReportVisible(false)} title="Report Question" hideCloseButton centered size="lg">
        <div className="flex flex-col space-y-4">
          <div className="flex items-center space-x-2">
            <IconHelpCircle size={24} />
            <p className="text-lg font-semibold">Are you sure you want to report this question?</p>
          </div>
          <p className="text-sm">Reporting this question will notify the admin and the question will be reviewed.</p>
          <Textarea placeholder="Enter your reason for reporting this question" className="w-full" />
        ...
        <div className="w-full mr-4">
            ...
            <ul className="flex">
              {data.map((res, i) => (
                <button
                  ...
                  } ${selectedSectionnumber == i ? "text-gray-50" : "text-gray-500"} font-semibold`}
          ...
          <div className="flex justify-end border-y-2 border-gray-300 pl-2 py-2 mt-2">
            <p className="py-2 font-semibold text-sm sm:text-lg mr-2 text-left">
              Marks for Correct Answer: <span className="text-green-600">{data[selectedSectionnumber]?.positiveMarks || ""}</span> |
              Negative Marks: <span className="text-red-500">{Math.abs(data[selectedSectionnumber]?.negativeMarks) || ""}</span>
            </p>
          </div>
          <div className="flex items-center border-b-2 pl-2 w-full">
            <div className="sm:flex w-full sm:items-center justify-between">
              <p className="font-bold text-md text-left sm:text-lg py-2">Q. {currentQuesIndex + 1}</p>
              <div className="flex items-center mr-2">
                ...
                <Tooltip
                  label={
                    (reviewQues && reviewQues[currentQuesIndex]?.bookmark) || bookmarks[currentQuesIndex]
                ...
                >
                  <button
                    ...
                    onClick={() => {
                      const ques = data[selectedSectionnumber].QuestionList[currentQuesIndex];
                      const bookmarkStatus = reviewQues[currentQuesIndex]?.bookmark || bookmarks[currentQuesIndex];
                      setBookmarks((draft) => {
          ...
          {Question.map((res, i) => {
            return currentQuesIndex == i ? (
              <div key={i} className="text-left justify-between overflow-y-scroll max-h-[450px] pl-2 w-full" style={{ height: "100%" }}>
                        ...
                        <ScrollArea h={450} className="p-4 pb-6">
                          <>
                              ...
                              <>
                                ...
                                {res.questionoption[0] &&
                                  res?.questionoption.map((ans, idx) => {
                                    return (
                                      <div key={idx} className="flex items-center space-x-3 cursor-pointer px-4 py-2 rounded-sm">
                                        <div
                                          className={`flex items-center py-2 px-4 rounded-md
																					${(reviewQues[i].answerStatus == "C" && idx == getRadio) || (res.correctoption == idx + 1 && showCorrectAns) ? "bg-green-100" : ""}
																					${reviewQues[i].answerStatus == "W" && idx == getRadio && "bg-red-100"}
																				`}
                                        >
                                          <input
                                            checked={idx == getRadio ? "checked" : null}
                                            type="radio"
                                            name={`ans` + i}
                                            disabled={true}
                                          ...
                                          <h3
                                            dangerouslySetInnerHTML={{
                                              __html: ans?.option.replace(/<br>/gi, "").replace(/&nbsp;/gi, ""),
                                            }}
                                            className="ml-4 mr-2 text-gray-900 text-md font-semibold text-left"
                                          />
                                          {(reviewQues[i].answerStatus == "C" && idx == getRadio) ||
                                          (res.correctoption == idx + 1 && showCorrectAns) ? (
                                            <i className="mr-1 fa fa-check text-green-500 text-2xl" />
                                          ) : (
                                            reviewQues[i].answerStatus == "W" &&
                                            idx == getRadio && <i className="mr-1 fa fa-times text-red-500 text-2xl" />
                                          )}
                                        </div>
                                      </div>
                                    );
                            ...
                            <div className="mt-4">
                              ...
                              {showCorrectAns ? (
                                getViewSolution && typeof getViewSolution == "object" ? (
                                  <div
                    ...
                    <>
                      ...
                      <div style={{ marginTop: "10px", minHeight: "200px" }}>
                        {showCorrectAns ? (
                          getViewSolution && typeof getViewSolution == "object" ? (
                            <>
                              <p className="text-xl font-semibold ml-5">Solution:</p>
                              <div
        ...
        <div className="border-gray-800 sm:border-l-2 border-t-2 sm:border-t-0 h-full">
          <div className="flex flex-col justify-center sm:w-80 items-center">
            <div className="w-full">
              ...
              <div className="flex items-center w-full py-3 px-2 text-gray-700 font-semibold">
                ...
                <p className="ml-1 text-sm">
                  {reviewRes?.section[selectedSectionnumber]?.question[currentQuesIndex]?.timeTaken ||
                  reviewRes?.section[selectedSectionnumber]?.question[currentQuesIndex]?.timeTaken?.length > 0
              ...
              <div className="flex items-center w-full py-3 px-2 text-gray-700 font-semibold">
                ...
                <p className="ml-1 text-sm">
                  {reviewRes?.section[selectedSectionnumber]?.avgSpentTime
                    ? `${reviewRes?.section[selectedSectionnumber]?.avgSpentTime?.toFixed(2)}s`
                    : "NA"}
                </p>
              </div>
              <Divider />
              <div className="flex items-center w-full py-3 px-2 text-gray-700 font-semibold">
                ...
                <p className="ml-1 text-sm">
                  {reviewRes?.section[selectedSectionnumber]?.avgCorrentSpentTime
                    ? `${reviewRes?.section[selectedSectionnumber]?.avgCorrentSpentTime?.toFixed(2)}s`
                    : "NA"}
                </p>
              </div>
              <Divider />
            ...
            <div className="flex flex-col w-full">
              ...
              <p className="text-left pl-2 mt-2 text-gray-700 font-semibold w-full">Choose a Question</p>
            </div>
            <div className="sm:h-[220px] overflow-y-scroll">
              <div className="flex grow flex-wrap max-w-[inherit] w-[100%] gap-4 p-4">
                {reviewRes?.section[selectedSectionnumber]?.question.map((ques, i) => (
                  <button
                    key={i}
                    onClick={() => {
                  ...
                  >
                    {!ques?.answerStatus && !ques?.notVisited ? (
                      <>
                        ...
                        <p className="absolute text-gray-50 top-[0%] -translate-x-2/4 left-2/4 translate-y-1/2">{i + 1}</p>
                      </>
                    ) : ques?.bothAnsReview ? (
                      <>
              ...
              <div className="flex flex-wrap gap-4 mt-2 items-center justify-center mb-20 sm:mb-5">
                  ...
                  <div className="flex flex-wrap font-medium items-center">
                    <div className="relative">
                      ...
                      <p className="absolute text-gray-50 top-[0%] -translate-x-2/4 left-2/4 translate-y-1">
                        {reviewRes?.section[selectedSectionnumber]?.question.filter((ques) => ques?.answerStatus).length || 0}
                      </p>
                    </div>
                  ...
                  <div className="flex flex-wrap font-medium items-center sm:ml-0 ml-6">
                    <div className="relative bg-[#ededed] border-[#979797] shadow-inner shadow-[#a9a9a962] border-2 flex flex-col items-center justify-center w-10 h-10 rounded-sm">
                      <p className="text-gray-800">
                        {reviewRes?.section[selectedSectionnumber]?.question.filter((ques) => ques?.notVisited).length || 0}
                      </p>
                    </div>
                  ...
                  <div className="flex flex-wrap font-medium items-center sm:ml-0 ml-4">
                    <div className="relative bg-[#765398] border-[#7a638f] shadow-inner shadow-[#d3a7fc] border-2 flex flex-col items-center justify-center w-10 h-10 rounded-full">
                      <p className="text-gray-50">
                        {reviewRes?.section[selectedSectionnumber]?.question.filter((ques) => ques?.markForReview).length || 0}
                      </p>
                    </div>
                ...
                <div className="flex font-medium items-center w-80 sm:ml-2 ml-6">
                  <div className="ml-2 relative bg-[#765398] border-[#7a638f] shadow-inner shadow-[#d3a7fc] border-2 flex flex-col justify-center items-center min-w-[2.5rem] min-h-[2.5rem] w-10 h-10 rounded-full">
                    <FaEquals className="absolute bottom-[-10%] font-normal text-sm right-[-10%] text-white bg-[#98c271] text-[8px] rounded-full min-w-[1rem] w-4 h-4 min-h-4" />
                    <p className="text-gray-50">
                      {reviewRes?.section[selectedSectionnumber]?.question.filter((ques) => ques?.bothAnsReview).length || 0}
                    </p>
                  </div>
                  <p className="text-sm font-normal ml-2">Answered & Marked for Review</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-around items-center sm:w-[30%] w-1/2">
            <button
              ...
              className={`bg-blue-500 hover:bg-blue-600 text-white rounded-sm px-4 py-2 ${
                currentQuesIndex == 0 && "bg-gray-400 hover:bg-gray-400 cursor-default"
              }`}
            >
              Previous
            </button>
            <button
              ...
              className={`bg-blue-500 hover:bg-blue-600 text-white rounded-sm px-4 py-2 ${
                currentQuesIndex == Question.length - 1 && "bg-gray-400 hover:bg-gray-400 cursor-default"
              }`}
            >
              Next
            </button>
